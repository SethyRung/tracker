...
describe("schemas", () => {
  ...
  describe("weightEntrySchema", () => {
    ...
    it("rejects out-of-range weights", () => {
      expect(() => weightEntrySchema.parse({ membership_id: "m1", weight_bps: -100 })).toThrow();
      expect(() => weightEntrySchema.parse({ membership_id: "m1", weight_bps: 15000 })).toThrow();
    });
  ...
  describe("sharePercentSchema", () => {
    it("accepts 0–10000", () => {
      ...
      expect(sharePercentSchema.parse(10000)).toBe(10000);
      expect(sharePercentSchema.parse(5000)).toBe(5000);
    });

    it("rejects out-of-range values", () => {
      expect(() => sharePercentSchema.parse(-100)).toThrow();
      expect(() => sharePercentSchema.parse(11000)).toThrow();
    });
  });
});
